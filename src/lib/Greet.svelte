<script lang="ts">
	...
	import { promptMessageStore } from './store';
	...
	function selectExample(example: Example) {
		promptMessageStore.set(example.question);
    onGreetHandler();
	}

	promptMessageStore.set('');
	let msg = '';
	let loading = false;
	let error: unknown;
	...
	async function onGreetHandler() {
		...
		loading = true;
		console.log('promptMessage', $promptMessageStore);

		await invoke('greet', { promptMessage: $promptMessageStore });
		// erase the prompt message after anser is generated
		promptMessageStore.set('');
	}
	...
	onMount(() => {
		document.addEventListener('keypress', ({ key }) => {
			if (key === 'Enter' && promptMessageStore) {
				onGreetHandler();
				console.log('Enter');
			}
		});
...
<div class="mx-3">
	...
	<label for="request" class="text-md/7 block my-2 font-medium text-gray-900 dark:text-gray-700"
		>Your Request</label
	>
	<input
		id="request"
		...
		bind:value={$promptMessageStore}
	/>

	<button
		disabled={!$promptMessageStore}
		on:click={onGreetHandler}
		type="button"
	...
	<h1>Choose a question example:</h1>
	...
	{#if $promptMessageStore}
		<p>You selected: {$promptMessageStore}</p>
		promptMessage: {$promptMessageStore}
	{/if}
